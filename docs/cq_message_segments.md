...
## /img：Gemini 生成图片并发送到 QQ

命令：

- 私聊：`/img 提示词` 或 `/img 1024x768 提示词`
- 群聊：`/img 提示词` 或 `/img --size 1024x768 提示词`（受群白名单与消息触发逻辑影响，但命令本身在群聊/私聊都可用）

实现要点：

- 使用 Gemini 原生图片生成模型：默认 `gemini-2.5-flash-image`
- 如果命令里写了 `宽x高`，会把尺寸要求写入生成提示，并在返回后把图片重采样到指定像素尺寸
- 调用 REST 接口：
  - `POST https://generativelanguage.googleapis.com/v1beta/models/<model>:generateContent`
  - Header：`x-goog-api-key: $GEMINI_API_KEY`
- 从返回 `candidates[].content.parts[].inlineData.data`（或 `inline_data.data`）取到 base64 图片，并以 CQ 码发送：
  - `[CQ:image,file=base64://...]`
- 同时会读取 `parts[].text` 作为说明文字；如果 Gemini 只返回文字也会照常发送。
