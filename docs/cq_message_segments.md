...
- 如果出现 `Cannot connect to Chrome at http://127.0.0.1:9223`，说明该端口没有 Chrome DevTools 在监听；重新运行 `scripts\start_gemini_search_mcp_cdp.bat`，不要关闭弹出的 Chrome，或先执行 `set GEMINI_SEARCH_CDP_PORT=9222` 换回默认端口
