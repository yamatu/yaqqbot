...
- `GEMINI_SEARCH_API_BASE` / `gemini_search_api_base`：gemini-search-mcp 的 OpenAI 兼容接口地址，例如 `http://127.0.0.1:8080`、`http://127.0.0.1:8080/v1` 或完整 `/v1/chat/completions`
- `GEMINI_SEARCH_API_KEY` / `gemini_search_api_key`：如果你的服务配置了鉴权则填写；没有可留空
- `GEMINI_SEARCH_MODEL` / `gemini_search_model`：默认 `gemini-search`
- 配置后任意 AI 模型对话都会自动附加联网搜索摘要；显式搜索可用 `/search`、`/news`
